...
* 루마 버텍스 MK / MK2: 고도화된 'EVV 스퍼터드 테크놀로지'가 적용된 이볼브 스퍼터드 메탈(반사) 필름입니다. 내 차를 더욱 돋보이게 만드는 미려하고 세련된 반사광을 통해 완벽한 프라이버시 보호 효과를 제공하며, 우수한 솔라 컨트롤 퍼포먼스(열 차단)로 최상의 승차감과 하차감을 동시에 완성합니다. 7년 품질 보증.
* 브이쿨 VK: 브이쿨의 기술력이 집약된 최고급 스퍼터링 라인업입니다. 우주 개발 과정에서 파생된 '선택적 태양광 투과 기술(XIR®)'이 적용되었으며, 금, 은, 인듐산화물 등 희귀 금속을 10겹의 다중 박막 구조로 코팅하여 강력한 내구성을 자랑합니다. 적외선 최대 98.4%, TSER 최대 74%의 압도적인 열 차단 성능과 맑은 시인성을 제공하는 최상급 필름입니다.
* 브이쿨 K: 100% 금속 스퍼터링 기술이 적용된 브이쿨의 베스트셀링 프리미엄 반사 필름입니다. 10겹의 멀티레이어 구조로 적외선을 95% 이상(TSER 최고 70%) 차단하며, 특히 부담스럽지 않고 은은한 반사 광택을 통해 도시적인 세련미와 탁월한 사생활 보호 기능을 제공합니다.
* 브이쿨 QB: 'Black Non-Reflective Technology'와 'Advanced Glazing Coating'이 적용된 가성비 뛰어난 프리미엄 비반사 필름입니다. 차분하고 고급스러운 블랙 색상으로 차량 본연의 디자인을 자연스럽게 살려주며, 가시광선 반사율이 낮아 시인성이 우수합니다. TSER 최대 63%. (※반사, 열반사 단어 사용 금지)
* 레인보우 IRIS 190: '다이아몬드 블랙' 색상의 슈퍼 나노 카본 세라믹 비반사 필름입니다. 전면 썬팅 기준 압도적으로 낮은 헤이즈 수치(0.98 이하)를 기록하여 타 브랜드 대비 2배 이상 넓고 쾌적한 전면 시야 개방감과 맑은 시인성을 제공합니다. 10년 품질 보증.
* 레인보우 IRIS V90: '하이퍼 골드 그린' 색상의 프리미엄 반사(스퍼터) 필름입니다. 태양 에너지를 초기부터 반사하여 강력한 열 차단을 제공하면서도, 금속성 필름의 단점인 주파수 간섭과 무아레 현상을 최소화했습니다. 역시 헤이즈 수치 0.98 이하의 맑은 시인성을 자랑하며 10년 품질 보증을 제공합니다.
* 틴트어카 포시즌 블랙(4SEASON BLACK): 수입차 B2B 프로모션으로 사랑받는 베스트셀링 제품으로, 차콜 색상의 은은한 매력과 함께 가격 대비 뛰어난 품질, 맑고 투명한 시인성을 제공하는 10년 보증 필름입니다.
* 틴트어카 비비드(VIVID): 합리적인 가격대에 선명한 시인성과 눈부심 방지 기능이 탁월한 가성비 틴팅 필름입니다. 5년 보증.
* 틴트어카 고스트(GHOST): 차콜 & 실버 색상의 세련된 반사 필름입니다. 무아레 현상을 최소화한 스퍼터 원단을 사용하여 프라이버시 보호와 높은 열 차단율을 동시에 만족시킵니다. 10년 보증.
* 틴트어카 볼레 프레스티지(Bolle Prestige): 뛰어난 시인성과 강력한 열 차단 성능을 보유한 틴트어카의 프리미엄 하이엔드 라인업으로, 시공 후 '평생 보증(Lifetime Warranty)'을 제공하는 독보적인 제품입니다.
* 레이노 팬텀 S (Phantom S): 고기능성 필름의 대중화를 이끈 베스트셀링 나노 카본 세라믹 필름입니다. S9 기준 최대 93%의 강력한 적외선 차단율을 자랑하며, 특수 나노 세라믹 코팅으로 변색과 스크래치에 매우 강한 내구성을 지녔습니다. 합리적인 가격에 10년 품질 보증을 제공합니다.
* 레이노 크로마 (CHROMA): 세계 최초 '나노 카본 스퍼터 기술'로 완성된 프리미엄 반사 필름입니다. '듀얼 컬러 컨트롤 기술'로 차량 내부와 외부의 컬러를 다르게 표현해 압도적인 고급스러움을 선사하며(크로마 9 미드나잇 블루 등), '듀얼 리플렉션 기술'로 내부 반사율을 줄여 눈이 편안한 시인성을 제공합니다. 10년 품질 보증.
* 레이노 팬텀 F (Phantom F): 진보된 초미립자 나노 카본 세라믹 설계로 필름 입자를 40% 미세화하여 극강의 선명한 시인성을 제공하는 하이엔드 비반사 필름입니다. F95 기준 최대 97%의 압도적인 열 차단 성능을 보여주며, 특히 99.9% 항균(안티박테리얼) 기술이 탑재되어 차량 내부를 세균으로부터 쾌적하게 지켜줍니다. 오닉스 블랙 색상, 10년 품질 보증.
"""
